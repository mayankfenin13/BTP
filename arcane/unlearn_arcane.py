...
def main():
    ...
    base_model = train_supervised(base_model, Subset(train_ds, keep), test_ds, epochs=epochs, batch_size=batch_size, lr=lr)
    ...
    if earliest is None:
        ...
        for bi in range(earliest, len(blocks)):
            ...
            used += blk
            model = train_supervised(model, Subset(train_ds, used), test_ds, epochs=epochs, batch_size=batch_size, lr=lr)
